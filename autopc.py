...
"""
    ...
    def onAISendMessageHandler(self):
    ...
    def getMessagesToken(self):
        ...
        for msg in self.messages:
            ...
            # 角色 + 内容 + 思考内容
            total += self._count_token(msg.role if hasattr(msg, "role") else "")
            total += self._count_token(msg.content if hasattr(msg, "content") else "")
            total += self._count_token(
                msg.reasoning_content if hasattr(msg, "reasoning_content") else ""
            )
    ...
    def recapMessages(self):
        textParts = []
        for msg in self.messages[1:]:
            role = msg.get("role") if isinstance(msg, dict) else msg.role
            content: Any = msg.get("content") if isinstance(msg, dict) else msg.content
            if role == "user":
                textParts.append(f"{role}:{content[0]}")
            else:
                textParts.append(f"{role}:{content}")
        rawText = "\n".join(textParts)

        recapMessages = [
            {"role": "system", "content": self.recapPrompt},
            {"role": "user", "content": [{"type": "text", "text": rawText}]},
        ]
        ...
        self.messages = [
            self.messages[0],  # 保留系统提示
            {"role": "user", "content": f"【历史对话】\n{summaryContent}"},
        ]
        print(f"[Token] 总结后内容: {summaryContent}")
        print("[Token] 总结完成")

    def tryRecapMessages(self):
        print("[压缩Token] 尝试总结上下文中")
        token = self.getMessagesToken()
        if token["token"] >= (int(self.config["context_window"]) * 0.9):
            self.fullMessages.append(
                {
                    "role": "assistant",
                    "name": "system_notice",
            ...
            self.onAISendMessageHandler()
            print("[压缩Token] 达到Token上限,开始总结")
            self.recapMessages()
            currentToken = self.getMessagesToken()
            self.fullMessages.append(
                {
                    "role": "assistant",
                    "name": "system_notice",
                    "content": f"Token压缩完成,总结前Token:{token['token']},当前Token:{currentToken['token']}",
                }
            )
            self.onAISendMessageHandler()
            print(
                f"[压缩Token] 总结完成,总结前Token:{token['token']},当前Token:{currentToken['token']}"
            )
        else:
            print("[压缩Token] 未满足总结条件")

    def downloadFile(self, controlArguments):
        try:
            print("[下载文件] 开始下载文件")
            savePath = os.path.expanduser(controlArguments["savePath"])
            # 发送 HTTP GET 请求，开启流式下载
            response = httpx.get(controlArguments["url"])
...
"""
                        ...
                        self.fullMessages = self.messages.copy()
            else:
    ...
    def sendImageToAI(self, prompt, imageSource, isLocal=True):
        if self.config["is_multimodal"]:
            ...
            self.fullMessages.append(newMessages)
        else:
        ...
        try:
            while finish_reason is None or finish_reason == "tool_calls":
                ...
                if message:
                    ...
                    self.fullMessages.append(message)
